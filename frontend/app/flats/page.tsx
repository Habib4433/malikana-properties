...
export default function FlatsPage() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/properties?sector=ফ্ল্যাট বিক্রয়`)
      .then(r => r.json())
      .then(d => { setFlats(d.data || []); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const filtered = flats
    .filter(f => type === "সব" || f.type === type)
    .filter(f => status === "সব" || f.status === status)
  ...
  return (
    <main style={{ fontFamily: "sans-serif", minHeight: "100vh", background: "#f4f7f5" }}>
      ...
      <section style={{ background: "linear-gradient(135deg,#0f2d1e,#1a5c34)", color: "#fff", padding: "40px 20px" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div style={{ fontSize: "13px", color: "#6ee7a0", marginBottom: "8px" }}>
            <a href="/" style={{ color: "#6ee7a0" }}>হোম</a> › ফ্ল্যাট বিক্রয়
          </div>
          <h1 style={{ fontSize: "clamp(22px,4vw,34px)", fontWeight: "700", marginBottom: "8px" }}>ফ্ল্যাট তালিকা</h1>
          <p style={{ color: "#9ecfb2", fontSize: "15px" }}>ঢাকাসহ সারাদেশে আধুনিক আবাসিক ফ্ল্যাটসমূহ</p>
        </div>
      </section>
      ...
      <div className="pad" style={{ maxWidth: "1260px", margin: "0 auto" }}>
        ...
        <p style={{ fontSize: "13px", color: "#666", marginBottom: "18px" }}>মোট <strong style={{ color: "#1a6b3c" }}>{filtered.length}টি</strong> ফ্ল্যাট পাওয়া গেছে</p>
            ...
            {filtered.map((f: any) => (
              <div key={f.id} className="card">
                <div style={{ position: "relative", height: "190px" }}>
                  ...
                  <span style={{ position: "absolute", top: "12px", right: "12px", background: f.status === "বুকড" ? "#6b7280" : "#16a34a", color: "#fff", fontSize: "11px", padding: "3px 10px", borderRadius: "20px" }}>{f.status}</span>
                </div>
                <div style={{ padding: "18px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                    ...
                    <span style={{ background: "#f0fdf4", color: "#166534", fontSize: "12px", padding: "3px 10px", borderRadius: "20px" }}>{f.type}</span>
                  </div>
                  <div style={{ fontSize: "20px", fontWeight: "700", color: "#1a6b3c", marginBottom: "8px" }}>৳ {f.price?.toLocaleString()}</div>
